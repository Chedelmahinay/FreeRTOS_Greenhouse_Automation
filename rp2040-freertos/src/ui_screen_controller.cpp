...
Created by: Ched Mahinay

This continuously integrating .cpp file is aimed to be built as the storage and handler of the
UI display of the user's screen of the ventilation system. The idea is to create a handler that would be interactive to
the user's input to the system (reads user input of c02 limit, Network SSID's, password)
and the display return in screen details to user's (View status).

Includes the ui displays:
...
Run this by changing the literal constants of preprocessor #ifdef for each display_task function.

.h file and this files location transfer to /src/project was discussed
*/
